...
func ParseSubscription(data []byte, opts ParseOptions) (proxy.Profile, error) {
	data = bytes.TrimSpace(data)
	if len(data) == 0 {
		return proxy.Profile{}, errors.New("subscription is empty")
	}
	...
	if opts.SubscriptionURL != "" || opts.SubscriptionName != "" || opts.SubscriptionID != "" {
		profile.Subscriptions = []proxy.Subscription{{
	...
	if len(groups) == 0 {
		groups = proxy.DefaultGroups(profile.ProxyNames())
	}
	if len(rules) == 0 {
...
func parseClashProxies(lines []string) ([]proxy.ProxyNode, error) {
...
func parseClashSettings(lines []string) proxy.Settings {
...
func parseClashProxyGroups(lines []string) ([]proxy.ProxyGroup, error) {
...
func proxyGroupFromItemLines(lines []string) (proxy.ProxyGroup, error) {
	...
	for _, line := range lines {
		...
		if !ok {
			return proxy.ProxyGroup{}, fmt.Errorf("unsupported proxy-group line %q", line)
		}
...
func parseClashRules(lines []string) []proxy.Rule {
	...
	for _, line := range lines {
		...
		rule := proxy.Rule{Type: strings.TrimSpace(unquote(parts[0]))}
...
func proxyFromFields(fields map[string]any) (proxy.ProxyNode, error) {
	name := stringField(fields, "name")
	typ := stringField(fields, "type")
	server := stringField(fields, "server")
	port, ok := intField(fields, "port")
	if !ok {
		return proxy.ProxyNode{}, fmt.Errorf("proxy %q has invalid port", name)
	}
