...
func TestLaunchConfigValidate(t *testing.T) {
	tests := []struct {
		...
		{name: "missing binary", conf: LaunchConfig{ConfigPath: "config.yaml", HomeDir: "mihomo"}},
		{name: "missing config", conf: LaunchConfig{BinaryPath: "mihomo.exe", HomeDir: "mihomo"}},
		{name: "missing home", conf: LaunchConfig{BinaryPath: "mihomo.exe", ConfigPath: "config.yaml"}},
	}
...
func TestLauncherRecordsUnexpectedExitAndRecovers(t *testing.T) {
	launcher := NewLauncher()
	starts := 0
	launcher.command = func(ctx context.Context, name string, args ...string) *exec.Cmd {
		starts++
		if starts == 1 {
			cmd := exec.CommandContext(ctx, os.Args[0], "-test.run=^TestLauncherHelperProcess$")
	...
	conf := LaunchConfig{BinaryPath: "mihomo.exe", ConfigPath: "config.yaml", HomeDir: "mihomo"}
