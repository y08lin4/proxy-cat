...
- Mihomo 启动器记录最近一次进程退出状态：PID、退出码、错误、退出时间，以及是否为预期停止
- 非预期退出后，状态刷新会把 Mihomo 退出错误收敛到应用 `LastError`
- 新增轻量恢复路径：状态刷新或显式恢复入口发现非预期退出、且仍保留有效启动配置时，可按原配置重新启动
- 手动停止 Mihomo 会标记为预期退出，避免误触发恢复路径
- auto-stable tick 增加 5 秒冷却，避免连续点击或重复调用造成过密健康检测
- auto-stable 冷却命中会返回可控结果，并写入状态与日志，便于 UI 告知用户
- 日志仍保持内存内最近 500 条的收敛策略，避免长期运行时无限增长
...
- `tsc --noEmit` 通过
- `vite build` 通过
- 单元测试覆盖 Mihomo 非预期退出记录、状态刷新恢复、显式恢复、手动停止不恢复
- 单元测试覆盖 auto-stable tick 冷却命中与状态记录

已知限制：

- 当前 Phase 4 是代码级稳定性收敛，不代表已完成真实端到端代理验收
- 当前本机只检测到 Go CLI，未检测到 `mihomo`、`wails`、`pnpm`
- 缺少 `mihomo.exe` 时，无法验证真实内核启动、健康检测、代理选择和网页访问链路
- 缺少 Wails v2 CLI 时，无法完成真实桌面端 smoke test
- 当前恢复能力是按最近一次有效启动配置进行轻量恢复，不是后台守护进程或无限重启策略
- auto-stable 冷却只限制应用层 tick 调用频率，不替代真实网络失败判断

验收状态：

- Mihomo 进程退出状态、恢复入口、auto-stable tick 冷却和日志收敛已有代码级闭环
- 真实长期运行稳定性仍需要在安装 Wails CLI、提供 `mihomo.exe`、加载有效订阅后做桌面端和网络端验收
